 ...
 * top-level VDEV into 64MB regions. For each region we maintain a reference
 * counter that is a sum of all reference counters of the cloned blocks that
 * have offsets within the region. This creates the regions array of 64bit
 * numbers for each top-level VDEV. The regions array is always kept in memory
 * and updated on disk in the same transaction group as the BRT updates to keep
 * everything in-sync. We can keep the array in memory, because it is very
 * small. With 64MB regions and 1TB VDEV the array requires only 128kB of memory
 * (we may decide to decrease the region size in the future). Now, when we want
 * to free a block, we first consult the array. If the counter for the whole
 * region is zero, there is no need to look for the BRT entry, as there isn't
 * one for sure. If the counter for the region is greater than zero, only then
 * we will do a BRT lookup and if an entry is found we will decrease the
 * reference counters in the entry and in the regions array.
 *
 ...
 * the dmu_brt_addref() function on the destination file. This function
 * allocates BPs for us. We iterate over all source BPs. If the given BP is
 * a hole or an embedded block, we just copy BP. If it points to a real data we
 * place this BP on a BRT pending list using the brt_pending_add() function.
 *
 ...
 * as when we log clone operation we cannot use the source object - it may
 ...
 * without mounting datasets and the source dataset is destroy before the
 * destination dataset is mounted and its ZIL replayed.
 * To address this situation we leverge zil_claim() mechanism where ZFS will
 ...
 * end up with leaked references in BRT. We address that too as ZFS gives as
...
/*
 * We divide each VDEV into 64MB chunks. Each chunk is represented in memory
 * by a 64bit counter, thus 1TB VDEV requires 128kB of memory.
 */
#define	BRT_RANGE_SIZE	(64 * 1024 * 1024)
/*
 ...
 * refcounts take 8MB of memory. If the block size is 4kB then we want to
 * have 2048 separate dirty bits. Each bit will represent change in one
...
typedef struct brt_vdev {
	...
	avl_tree_t	bv_tree;
	/*
	 * Does the bv_refcount[] array needs byte swapping?
	 */
...
static uint64_t
brt_vdev_refcount_get(const brt_vdev_t *brtvd, uint64_t idx)
{

	ASSERT3U(idx, <, brtvd->bv_size);

	if (brtvd->bv_need_byteswap) {
		return (BSWAP_64(brtvd->bv_refcount[idx]));
	} else {
		return (brtvd->bv_refcount[idx]);
	}
}

static void
brt_vdev_refcount_set(brt_vdev_t *brtvd, uint64_t idx, uint64_t refcnt)
{

	ASSERT3U(idx, <, brtvd->bv_size);

	if (brtvd->bv_need_byteswap) {
		brtvd->bv_refcount[idx] = BSWAP_64(refcnt);
	} else {
		brtvd->bv_refcount[idx] = refcnt;
	}
}

static void
brt_vdev_refcount_inc(brt_vdev_t *brtvd, uint64_t idx)
{
	uint64_t refcnt;

	ASSERT3U(idx, <, brtvd->bv_size);

	refcnt = brt_vdev_refcount_get(brtvd, idx);

	brt_vdev_refcount_set(brtvd, idx, refcnt + 1);
}

static void
brt_vdev_refcount_dec(brt_vdev_t *brtvd, uint64_t idx)
{
	uint64_t refcnt;
	...
	for (vdevid = 0; vdevid < brt->brt_nvdevs; vdevid++) {
		...
		if (brtvd->bv_totalcount > 0) {
			...
			for (idx = 0; idx < brtvd->bv_size; idx++) {
				if (brt_vdev_refcount_get(brtvd, idx) > 0) {
					printf("      [%04llu] %llu\n", idx,
					    brt_vdev_refcount_get(brtvd, idx));
				}
			}
		}
		if (brtvd->bv_dirty) {
...
brt_vdev_lookup(brt_t *brt, brt_vdev_t *brtvd, const brt_entry_t *bre)
{
	...
	if (brtvd->bv_refcount != NULL && idx < brtvd->bv_size) {
		...
		return (brt_vdev_refcount_get(brtvd, idx) > 0);
	}
    ...
    uint64_t dsize)
{
	...
	brtvd->bv_totalcount++;
	brt_vdev_refcount_inc(brtvd, idx);
	brtvd->bv_dirty = TRUE;
    ...
    uint64_t dsize)
{
	...
	brtvd->bv_totalcount--;
	brt_vdev_refcount_dec(brtvd, idx);
	brtvd->bv_dirty = TRUE;
	idx = idx / brt->brt_blocksize / 8;
	BT_SET(brtvd->bv_bitmap, idx);
...
brt_get_used(spa_t *spa)
{
	...
	return (brt->brt_drefsize);
}

uint64_t
brt_get_logical_used(spa_t *spa)
{
	brt_t *brt = spa->spa_brt;
	...
	return (brt->brt_drefsize + brt->brt_dsize);
}
