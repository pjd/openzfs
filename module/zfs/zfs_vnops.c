    ...
    uint64_t *outoffp, uint64_t *lenp, cred_t *cr)
{
	 ...
	 * See zfs_zaccess_common()
	 */
	if ((outzp->z_pflags & ZFS_IMMUTABLE)) {
		zfs_exit_two(inzfsvfs, outzfsvfs, FTAG);
		return (SET_ERROR(EPERM));
	}

	if (inzfsvfs->z_log && inos->os_sync == ZFS_SYNC_ALWAYS) {
		zil_commit(inzfsvfs->z_log, inzp->z_id);
	}
	 ...
	 */
	if (inzp == outzp) {
		if ((inoff >= outoff && inoff < outoff + len) ||
		    (outoff >= inoff && outoff < inoff + len)) {
			zfs_exit_two(inzfsvfs, outzfsvfs, FTAG);
			return (SET_ERROR(EINVAL));
		}
	}
	 ...
	 */
	if (inblksz != outzp->z_blksz && outzp->z_size > 0) {
	 ...
	 */
	if ((inoff % inblksz) != 0 || (outoff % inzp->z_blksz) != 0) {
		error = SET_ERROR(EXDEV);
		goto unlock;
	}
	/*
	 ...
	 */
	if ((len % inblksz) != 0 && len != inzp->z_size - inoff) {
		error = SET_ERROR(EXDEV);
		goto unlock;
	}
	...
	bps = kmem_alloc(sizeof (bps[0]) * BRT_NBLOCKS, KM_SLEEP);
	 ...
	 */
	while (len > 0) {
		size = MIN(inblksz * BRT_NBLOCKS, len);
		...
		DB_DNODE_ENTER(db);
		dmu_tx_hold_write_by_dnode(tx, DB_DNODE(db), outoff, size);
		DB_DNODE_EXIT(db);
		...
		if (outlr->lr_length == UINT64_MAX) {
			...
			zfs_rangelock_reduce(outlr, outoff, len);
		}
	...
	kmem_free(bps, sizeof (bps[0]) * BRT_NBLOCKS);
