...
static int
zfs_znode_cache_constructor(void *buf, void *arg __unused, int kmflags __unused)
{
	znode_t *zp = buf;
...
static void
zfs_znode_cache_destructor(void *buf, void *arg __unused)
{
	znode_t *zp = buf;
    ...
    dmu_object_type_t obj_type, sa_handle_t *hdl)
{
	...
	SA_ADD_BULK_ATTR(bulk, count, SA_ZPL_GEN(zfsvfs), NULL, &gen, 8);
	...
	if (sa_bulk_lookup(zp->z_sa_hdl, bulk, count) != 0 || gen == 0 ||
...
zfs_create_fs(objset_t *os, cred_t *cr, nvlist_t *zplprops, dmu_tx_t *tx)
{
	...
	VERIFY0(zfs_acl_ids_create(rootzp, IS_ROOT_NODE, &vattr, cr, NULL,
	    &acl_ids, NULL));
#else
	memset(&acl_ids, 0, sizeof (acl_ids));
#endif
printf("%s:%u\n", __func__, __LINE__);
